...
class Action:
    ...
    def is_wrongop(self):
        ...
        -------
        bool
            True if action is a wrongop action, otherwise False
        """
        return isinstance(self, WrongOp)
...
class NotDiscoveredOp(Action):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(name="notdiscoveredop",
                         target=(1, 0),
                         cost=100, #Before was 0. FL
                         prob=1.0,
                         req_access=AccessLevel.NONE)
...
class ParameterisedActionSpace(spaces.MultiDiscrete):
    ...
    def get_action(self, action_vec, current_state):
        ...
        2. if action tries to reach a not discovered host, WrongOperation
           is returned.
        """
        ...
        # If the target has not been discovered yet,
        # return that it is a wrong operation
        if not current_state.host_discovered(target):   #FL
            return WrongOp()  #FL
